...
function BanksPage() {
	...
	return (
		<div className="px-7 py-6">
			...
			{/* Status banner */}
			{!cert ? (
				<div className="flex items-center gap-3 px-3 py-2.5 rounded-[3px] bg-transparent text-error-text border border-[rgba(255,155,155,0.3)] mb-5 font-mono" style={{ fontSize: 12, letterSpacing: "0.04em" }}>
				...
				<div className="flex items-center gap-3 px-3 py-2.5 rounded-[3px] bg-transparent text-warning-text border border-warning-border mb-5 font-mono" style={{ fontSize: 12, letterSpacing: "0.04em" }}>
					<Icons.AlertTriangle size={14} className="flex-shrink-0" />
					<span className="flex-1">{totalNeedReauth} ACCOUNT{totalNeedReauth > 1 ? "S" : ""} NEED RE-AUTHENTICATION — consent expiring within 14 days.</span>
				</div>
			) : (
				<div className="flex items-center gap-3 px-3 py-2.5 rounded-[3px] bg-transparent text-notice-text border border-notice-border mb-5 font-mono" style={{ fontSize: 12, letterSpacing: "0.04em" }}>
					<Icons.Shield size={14} className="flex-shrink-0" />
					<span>ALL CONNECTIONS HEALTHY · {connectedBankGroups.flatMap(g => g.accounts).length} accounts active · certificate valid until {cert.expires}</span>
				</div>
			)}
